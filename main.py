...
tabela_preco_condicao = pd.DataFrame(
    columns=["cod_cliente", "sku", "nome_razao_social", "condicao", "preco_condicao_0"]
)


def calcula_preco_condicao_fator(
    condicao_cliente: int, condicao_atual: int, preco_base: float
):
...
def cria_novo_registro(cod_produto, cod_cliente, condicoes):
    ...
    for index, row in condicoes.iterrows():
        new_row[
            "preco_condicao_" + str(int(row["condicao"]))
        ] = calcula_preco_condicao_fator(
            condicao_cliente.values[0], row["condicao"], preco_base.values[0]
        )
    return new_row


def estrutura_tabelas_db(tabela_local, nome_tabela_db: str):
    tabela_existe = pd.read_sql_query(
        f'''SELECT name FROM sqlite_master
            WHERE type="table" AND name="{nome_tabela_db}"''',
        con,
    )
    ...
    # se existe e colunas estão iguais
    if list(tabela_local.columns) == db_col_names:
        return True
    else:
        adiciona_colunas_db(tabela_local, nome_tabela_db)


def adiciona_colunas_db(tabela_local, nome_tabela_db: str) -> None:
    cur = con.execute(f"select * from '{nome_tabela_db}'")
    db_col_names = [description[0] for description in cur.description]
...
while continuar == "s":
    try:
        cod_produto = input("Digite o código do produto (sku): ")
    ...
    tabela_preco_condicao = pd.concat(
        [tabela_preco_condicao, novo_registro], ignore_index=True
    )
    ...
    if continuar == "n":
        ...
        if persistir == "s":
            ...
            try:
                if estrutura_tabelas_db(tabela_preco_condicao, "preco_condicao_pedido"):
                    tabela_preco_condicao.to_sql(
                        "preco_condicao_pedido", con, if_exists="append", index=False
                    )
